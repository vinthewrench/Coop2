...
typedef void (*CmdCallFunct)(void* arg, CmdParser *cmdParser);
typedef void (*ErrCallFunct)(void* arg, CmdBufferObject *cmdBuffer);
...
public:
	 ...
	 * @param cmdBuffer         Buffer object for data handling
	 * @param serial            Arduino serial interface from comming data
	 * @return             A char was processed

	 */
	bool updateCmdProcessing(CmdParser *cmdParser, CmdBufferObject *cmdBuffer,
									 Stream *serial);
	
	void addErrorHandler(ErrCallFunct errFunct);
	void addCompletionHandler(CmdCallFunct compFunct);
	...
	virtual const char* cmdName(size_t idx);
	virtual size_t cmdCount();

	void* 			_cBArg;
	bool			_busyInCallback;
	

private:
	ErrCallFunct _errCB;
	CmdCallFunct _compCB;		// call this when we are done for any async output
...
public:
	...
	virtual size_t cmdCount(){
		...
		size_t count = 0;
		for(size_t idx = 0; idx < STORESIZE; idx++)
			if (this->m_cmdList[idx] != NULL)  count++;
	...
	virtual bool callStoreFunct(size_t idx, CmdParser *cmdParser)
	{
		if (idx < STORESIZE && m_functList[idx] != NULL) {
			m_functList[idx](_cBArg, cmdParser);
			return true;
		}
