...
bool INA219::begin(uint8_t address, TwoWire &wirePort)
{
...
int16_t INA219::getBusVoltage_raw() {
  uint16_t value;
	...
	value = readDoubleRegister(INA219_REG_BUSVOLTAGE);
...
int16_t INA219::getCurrent_raw() {
  ...
  // value even if it's an unfortunate extra step
	...
	writeDoubleRegister(INA219_REG_CALIBRATION, _calValue);
 
//  // Now we can safely read the CURRENT register!
	value = readDoubleRegister(INA219_REG_CURRENT);
...
int16_t INA219::getPower_raw() {
  ...
  // Now we can safely read the POWER register!
	value = readDoubleRegister(INA219_REG_POWER);
  return value;
}
...
int16_t INA219::getShuntVoltage_raw() {
  uint16_t value;
	...
	value = readDoubleRegister(INA219_REG_SHUNTVOLTAGE);
  return value;
}
...
//	// Set Calibration register to 'Cal' calculated above
	...
	writeDoubleRegister(INA219_REG_CONFIG, config);
}
...
bool INA219::writeDoubleRegister(INA219_Register reg, uint16_t data)
{
	 _i2cPort->beginTransmission(_deviceAddress);
	 _i2cPort->write(reg);
...
uint16_t INA219::readDoubleRegister(INA219_Register reg)
{ //big endian
	 _i2cPort->beginTransmission(_deviceAddress);
	 _i2cPort->write(reg);
